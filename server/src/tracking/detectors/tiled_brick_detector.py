...
class TiledBrickDetector(object):
    ...
    def find_brick_among_tiles(self, tiled_board_area, coordinates, board_snapshot, debug=False):
        """
        ...
        # Extract tile strip
        tile_strip_image = tiled_board_area.tile_strip(coordinates, board_snapshot, grayscaled=True)

        # Calculate medians
        medians = self.medians_of_tiles(tile_strip_image, coordinates, tiled_board_area, board_snapshot)
        ...
        if debug:
            ...
            cv2.imshow('Area image', board_snapshot.board_image('MEDIUM'))
        ...
        probabilities = self.probabilities_of_bricks(tile_strip_image, coordinates, tiled_board_area, board_snapshot)
    ...
    def find_bricks_among_tiles(self, tiled_board_area, coordinates, board_snapshot, debug=False):
        """
    ...
    def medians_of_tiles(self, tile_strip_image, coordinates, tiled_board_area, board_snapshot):
        return [self.median_of_tile(i, tile_strip_image, tiled_board_area, board_snapshot) for i in range(0, len(coordinates))]

    def median_of_tile(self, index, tile_strip_image, tiled_board_area, board_snapshot):

        # Extract brick image
        brick_image = tiled_board_area.tile_from_strip_image(index, tile_strip_image, board_snapshot)

        # Remove border
        tile_width, tile_height = tiled_board_area.tile_size(board_snapshot)
    ...
    def probabilities_of_bricks(self, tile_strip_image, coordinates, tiled_board_area, board_snapshot):
        return [self.probability_of_brick(i, tile_strip_image, tiled_board_area, board_snapshot) for i in range(0, len(coordinates))]

    def probability_of_brick(self, index, tile_strip_image, tiled_board_area, board_snapshot):
